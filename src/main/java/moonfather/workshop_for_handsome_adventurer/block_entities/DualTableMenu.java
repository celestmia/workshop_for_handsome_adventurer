...
public class DualTableMenu extends SimpleTableMenu
{
    ...
    public DualTableMenu(int containerId, Inventory inventory, ContainerLevelAccess levelAccess, @Nullable MenuType<?> menuType)
    {
        ...
        for (int k = CUST_SLOT_START; k <= CUST_SLOT_END; k++)
        {
            ...
            if (k == CUST_SLOT_START)
            {
                y0 = current.y - 9;
            }
            if ((k - CUST_SLOT_START) >= this.getCustomizationSlotCount())
            {
                continue;
    ...
    {
        return OptionsHolder.COMMON.DualTableNumberOfSlots.get();
    }
    ...
    public boolean stillValid(Player player)
    {
        ...
        {
            if (!(level.getBlockState(pos).getBlock() instanceof DualTableBaseBlock))
            {
                return false;
            }
    ...
    {
        return index == RESULT_SLOT || index == SECONDARY_RESULT_SLOT;
    }
